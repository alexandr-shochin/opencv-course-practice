...
int main(int argc, char* argv[])
{
	...
	{
		KeyPoint objectPnt = keypoints1[matches[matchInd].queryIdx];
		KeyPoint scenePnt  = keypoints2[matches[matchInd].trainIdx];

		Point2f difference = (Point2f)center - objectPnt.pt;
		float distance = (float)norm(difference);
		float angle = atan(difference.y / difference.x);
		float dAngle = (scenePnt.angle - objectPnt.angle) * (float)CV_PI / 180.0f;
		float dScale = scenePnt.size / objectPnt.size;
		Point center = Point(cvRound(scenePnt.pt.x + cos(angle + dAngle) * (distance * dScale)), 
							 cvRound(scenePnt.pt.y + sin(angle + dAngle) * (distance * dScale)));
	...
	//finding homography
	vector<Point> srcmat(matches.size());
	vector<Point> dstmat(matches.size());
	for (size_t matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		srcmat[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
		dstmat[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
	}
	Mat homography = findHomography(srcmat, dstmat, CV_RANSAC, ransacThresh);
